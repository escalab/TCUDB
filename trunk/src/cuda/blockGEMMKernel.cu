        ...
        float *C, int ldc)
{
	...
	printf("overflowB: %d\n", overflowB);
	printf("overflowA: %d\n", overflowA);
	cudaMalloc((void**) &b, sizeof(half) * subCols * k);
	for(int i = 0; i < numStreams; ++i){
		cublasCreate(&handles[i]);
		cudaStreamCreate(&streams[i]);
		cudaMalloc((void**) &a[i], sizeof(half) * subRows * k);
		cudaMalloc((void**) &c[i], sizeof(float) * subCols * subRows);
        // allocate pinned memory on the CPU
		cudaMallocHost((void**) &a_h[i], sizeof(half) * subRows * k);
	...
	cudaMallocHost((void**) &temp3, sizeof(half)*subCols * k );
	for(unsigned long long i = 0; i < numSubMatrixB + 1; ++i){
		...
		cudaMemcpyAsync(b, temp3, sizeof(half)*subCols*k, cudaMemcpyHostToDevice, streams[0]);
		...
		while(y < numSubMatrixA + 1){
			...
			cudaMemcpyAsync(a[y % numStreams], a_h[y % numStreams], sizeof(half)*subRows*k, cudaMemcpyHostToDevice, streams[y % numStreams]);
			printf("sending multiply %d,%d to stream %d\n", y, i, y % numStreams);
            printf("perform cublasGemmEx with FP16 inputs\n");
