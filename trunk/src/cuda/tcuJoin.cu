...
__global__ void static gpu_denseFill(char *mat_i, char *mat_j, char *mat_val, 
...
/* For MSplit */
__host__ void static denseFill(struct joinNode *jNode,
                            //    char *row, char *col, char *val, 
                               int nelem, float *mat, int dim) 
{

    for (int i = 0; i < nelem; i++) {
        int index = i * sizeof(int);
        // parse 4 bytes at a time
        int *row = (int*)&jNode->leftTable->content[jNode->leftKeyIndex][index];
        int *col = (int*)&jNode->leftTable->content[jNode->leftOutputIndex[0]][index];
        int *val = (int*)&jNode->leftTable->content[jNode->leftOutputIndex[1]][index];
        float value = (float)*val;

        mat[(*row) * dim + (*col)] = value;
    }
...
/* For MSplit */
__host__ void static denseFill_transpose(struct joinNode *jNode,
                                         int nelem, float *mat, int dim)
{
    for (int i = 0; i < nelem; i++) {
        int index = i * sizeof(int);
        // parse 4 bytes at a time
        int *row = (int*)&jNode->rightTable->content[jNode->rightOutputIndex[0]][index];
        int *col = (int*)&jNode->rightTable->content[jNode->rightKeyIndex][index];
        int *val = (int*)&jNode->rightTable->content[jNode->rightOutputIndex[1]][index];
        float value = (float)*val;
...
__global__ void static gpu_denseFill_transpose(char *mat_i, char *mat_j, 
...
__host__ static void getValIndex(struct joinNode *jNode, struct groupByNode *gb, 
    ...
    for (int i = 0; i < jNode->leftOutputAttrNum; i++) {
        ...
        // Note: hard to know which colIdx belong to sum() as value or use as one dimension
        if (ldata2 == -1) {
        ...
        int fillOne)
{
    ...
    for (int i = 0; i < tupleNum; i++) {
        int *key = (int*)&joinKey[i * sizeof(int)];
        // Note: these two lines require atomic operations to run on GPU
...
// #endif
    ...
    A_h = (float*)calloc(MATRIX_K * MATRIX_K, sizeof(float));
    B_h = (float*)calloc(MATRIX_K * MATRIX_K, sizeof(float));
    C_h = (float*)malloc( sizeof(float) * MATRIX_K * MATRIX_K);
    ...
    if (gb && gbConstant != 1 && gb->numFuncExpCol == 2 && gb->math_op == MULTIPLY) {
        CUDA_SAFE_CALL_NO_SYNC(cudaMalloc((void**)&gpu_a_col,foreignKeySize));
        CUDA_SAFE_CALL_NO_SYNC(cudaMalloc((void**)&gpu_b_row,primaryKeySize));
    ...
    else if (gb && gb->numFuncExpCol == 2 && gb->math_op == DIVIDE) {
        CUDA_SAFE_CALL_NO_SYNC(cudaMalloc((void**)&gpu_ldata,foreignKeySize));
        CUDA_SAFE_CALL_NO_SYNC(cudaMalloc((void**)&gpu_rdata,primaryKeySize));
    }
    ...
    // no need to cudaMemcpy, but read data from table
    denseFill(jNode,
              MATRIX_M, 
              A_h, 
              MATRIX_K);
    
    denseFill_transpose(jNode,
              MATRIX_N,
              B_h,
              MATRIX_K);
    ...
    if (gb && gbConstant != 1 && gb->numFuncExpCol == 2 && gb->math_op == MULTIPLY) {
        CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(gpu_a_col,jNode->leftTable->content[jNode->leftOutputIndex[0]], foreignKeySize,cudaMemcpyHostToDevice));
        CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(gpu_b_row,jNode->rightTable->content[jNode->rightOutputIndex[0]], primaryKeySize,cudaMemcpyHostToDevice));
        CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(gpu_ldata,jNode->leftTable->content[jNode->leftOutputIndex[1]], foreignKeySize,cudaMemcpyHostToDevice));
        CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(gpu_rdata,jNode->rightTable->content[jNode->rightOutputIndex[1]], primaryKeySize,cudaMemcpyHostToDevice));
        ...
        CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(gpu_ldata,jNode->leftTable->content[jNode->leftOutputIndex[0]], foreignKeySize,cudaMemcpyHostToDevice));
        CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(gpu_rdata,jNode->rightTable->content[jNode->rightOutputIndex[0]], primaryKeySize,cudaMemcpyHostToDevice));
        // printf("micro_q4 cudaMemcpy ldata, rdata\n");
    }
#endif // end of dense_block_gemm
    ...
    // assume sparsity, A_nnz, B_nnz is given
        if (gb && gb->gbExp[gb->aggFuncIndex].func == SUM) 
        {
            ...
            else if (gb && gbConstant != 1 && gb->numFuncExpCol == 2 && gb->math_op == MULTIPLY)
            {
                // assume square matrices for simplicity
                printf("Test simpleMM.sql pattern\n");
                // judge tupleNum (size) here to call block or regular cublasGemmEx
#ifdef BLOCK_HALF
                
                unsigned long matArow, matAcol, matBrow, matBcol;
                matArow = MATRIX_K;
                matAcol = MATRIX_K;
                matBrow = MATRIX_K;
                matBcol = MATRIX_K;
                
                struct gpu_timer msplit_kernel_start;
                msplitm('N', 'N', 
                ...
                msplit_kernel_time = msplit_kernel_start.milliseconds_elapsed();

                printf("msplit data movement:       %f ms\n", cudaMemcpyTime);
                printf("msplit kernel time:       %f ms\n", msplit_kernel_time);
                return res;
#else
                // printf("using gpu_dense_fill and cublasGemmEx\n");
                struct gpu_timer fillStart;
                gpu_denseFill<<<(MAX_THREADS+leftTupleNum-1)/MAX_THREADS,MAX_THREADS>>>(gpu_fact, gpu_a_col, 
                    gpu_ldata, 
                    MATRIX_K, d_fp16_A, leftTupleNum, 
                    jNode->leftTable->attrType[jNode->leftKeyIndex]);
                
                cudaErrCheck(cudaFree(gpu_fact));
                cudaErrCheck(cudaFree(gpu_a_col));

                gpu_denseFill<<<(MAX_THREADS+rightTupleNum-1)/MAX_THREADS,MAX_THREADS>>>(
                    gpu_b_row, gpu_dim, 
                    gpu_rdata, 
                    MATRIX_K, d_fp16_BT, rightTupleNum, 
                    jNode->rightTable->attrType[jNode->rightKeyIndex]);
                
                fillTime = fillStart.milliseconds_elapsed();
                cudaErrCheck(cudaFree(gpu_dim));
                cudaErrCheck(cudaFree(gpu_b_row));
                
                printf("MxK:%dx%d\tKxN:%dx%d\n", MATRIX_K, MATRIX_K, MATRIX_K, MATRIX_K);
                printf("Running cublasGemmEx on TCUs...\n");
                struct gpu_timer compute_start;
                cudaErrCheck(cudaEventRecord(startcublasEX));

                // square matrices
                cublasErrCheck(cublasGemmEx(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N,
                    MATRIX_K, MATRIX_K, MATRIX_K,
                    &alpha,
