...
void tbl2csr_transpose_pr(int tupleNum, char *joinKey, char *Xdata,
                          int *csrOffsets, int *csrColumns, float *csrValues,
                          int num_rows, int fillOne)
{
    ...
    for (int i = 0; i < tupleNum; i++) {
        ...
        if (fillOne) {
            ...
            int *data = (int*)&Xdata[i * sizeof(int)];
            //printf("B val: %f\n", *data);
            csrValues[offset] = (float)1.0/(*data);
            // printf("r_degree: %f\n", csrValues[offset]);
        }
    }
}
...
__global__ void gpu_tbl2csr_pr(int tupleNum, char *joinKey, char *Xdata,
                               int *csrOffsets, int *csrColumns, float *csrValues,
                               int fillOne)
{
    ...
    if (fillOne) {
        ...
        //float *data = (float*)&Xdata[i * sizeof(float)];
        // float *rank = (float*)&ldata[i * sizeof(float)];
        float *data = (float*)&Xdata[i * sizeof(int)];
        csrValues[i] = (float) PAGERANK_ALPHA * (*data);
        // printf("rank: %f\n", csrValues[i]);
    }
...
void tcuspmm_pr(int Annz, int A_num_rows, int A_num_cols,
             int Bnnz, int B_num_rows, int B_num_cols,
             int MATRIX_K, int foreignKeySize,
             int leftTupleNum, char *gpu_fact, char *ldata,
             int rightTupleNum, char *dim, char *rdata)
{
    ...
    tbl2csr_transpose_pr(rightTupleNum, dim, rdata,
                         hB_csrOffsets, hB_csrColumns, hB_csrValues,
                         MATRIX_K, 0);
    ...
    gpu_tbl2csr_pr<<<(MAX_THREADS+leftTupleNum-1)/MAX_THREADS, MAX_THREADS>>> (
            leftTupleNum, gpu_fact, gpu_ldata,
            dA_csrOffsets, dA_csrColumns, dA_csrValues, 0); 
    clock_gettime(CLOCK_REALTIME, &fill_end);
    ...
    // debug_end.tv_nsec - debug_start.tv_nsec;
    // printf("cusparse handle init time %f ms\n", debug_elapse / MILLION);
}
